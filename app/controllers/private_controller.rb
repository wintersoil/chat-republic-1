class PrivateController < ApplicationController
  ...
  def new
    ...
    @recipient = params[:id]
    @private_messages = PrivateMessage.where(user: current_user, recipient: @recipient).merge(PrivateMessage.where(user: User.find_by(@recipient), recipient: current_user.id))
  end
